...
def crear_juego(pieza_inicial):
    ...
    """
    return crear_grilla(ANCHO_JUEGO, ALTO_JUEGO), trasladar_pieza(pieza_inicial, ANCHO_JUEGO // 2, 0), False
...
def mover(juego, direccion):
    ...
    for x, y in pieza_movida:
        if 0 > x or x >= ANCHO_JUEGO or hay_superficie(juego, x, y):
            return juego
    return juego[0], pieza_movida, terminado(juego)


def consolidar_pieza(juego):
    grilla = juego[0]
    pieza_consolidada = juego[1]
    for x, y in pieza_consolidada:
        grilla[y][x] = 2
    return grilla, pieza_consolidada, terminado(juego)
...
def eliminar_filas(juego):
    ...
    for fila in range(len(grilla)):
        if grilla[fila].count(2) == ANCHO_JUEGO:
            grilla.pop(fila)
            grilla.insert(0, fila_vacia)
    return grilla, juego[1], juego[2]
...
def avanzar(juego, siguiente_pieza):
    ...
    **NOTA:** Hay una simplificación respecto del Tetris real a tener en
    ...
    if terminado(juego):
        return juego, False
    pieza_descender = trasladar_pieza(pieza_actual, 0, 1)
    for x, y in pieza_descender:
        if y == ALTO_JUEGO or hay_superficie(juego, x, y):
            juego = consolidar_pieza(juego)
            juego = eliminar_filas(juego)
            return (grilla, trasladar_pieza(siguiente_pieza, ANCHO_JUEGO // 2, 0), terminado(juego)), True
    return (grilla, pieza_descender, terminado(juego)), False
